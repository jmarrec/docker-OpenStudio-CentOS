...
RUN yum -y update &&\
    ...
    yum install -y gcc10-gcc-c++ python3 patch git make wget redhat-lsb-core &&\
    ...
    mkdir ~/Qt && cd ~/Qt &&\
    yum install -y p7zip xcb-util-wm xcb-util-image xcb-util-keysyms xcb-util-renderutil libxkbcommon-x11 &&\
